...
class RequestMoneyController extends APIController
{
    ...
    public function retrieve(Request $request){
      ...
      $response = array();
      if($data['value'] != null){
        $result = RequestMoney::where('status', '=', 0)->where($data['column'], 'like', $data['value']->offset(intval($data['offset'])))->limit(intval($data['limit']))->orderBy($data['sort']['column'], $data['sort']['value'])->get();
      }else{
        $result = RequestMoney::where('status', '=', 0)->where($data['column'], 'like', $data['value'])->orWhere('type', '<=', 100)->offset(intval($data['offset']))->limit(intval($data['limit']))->orderBy($data['sort']['column'], $data['sort']['value'])->get();
      }
